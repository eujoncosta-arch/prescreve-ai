...
5. ✅ Consolidar `repositorio`/`biblioteca`/`evidencias`/`evidence`/`farmalib`/`eurofarma` (6 páginas de catálogo/evidência sobrepostas) em quantas páginas? → **Contagem-alvo: 6 → 4** (evidência 3→2: `/evidence` fica como principal, `/evidencias` continua separada por ser governança de versão, não biblioteca; catálogo 3→2: `/biblioteca` fica como principal, `/eurofarma` continua separada por ser dashboard de sync, não catálogo). Decisão registrada em `docs/RM-74-CATALOG-EVIDENCE-CONSOLIDATION-DECISION.md` (2026-08-04) — a migração de dado real (`/repositorio` e `/farmalib` dobrando) fica para uma RM futura dedicada, por envolver reconciliação de conteúdo curado real (esforço "Alto", nenhuma remoção de nav feita ainda).
