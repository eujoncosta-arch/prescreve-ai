...
const ALTERNATIVAS_DB: Record<string, AlternativasDB> = {
  ...
  'E78.5': {
    condicao: 'Dislipidemia / Hipercolesterolemia',
    nota: 'Meta LDL-c: < 55 mg/dL em muito alto risco CV, < 70 mg/dL em alto risco, < 100 mg/dL em risco intermediário. "Lower is better" — cada 1 mmol/L de redução de LDL reduz MACE em 22% (CTT). Estatina de alta intensidade é base de toda terapia.',
    alternativas: [
      { linha: '1a_linha', molecula: 'Atorvastatina 40–80mg', classe: 'Estatina alta intensidade', indicacao: 'Alto/muito alto risco CV, pós-SCA, dislipidemia com DCV', nnt: 20, forca_evidencia: 'A', diretriz: 'ESC/EAS 2019 / SBC 2023', disponivel_sus: true, custo_estimado_mes: 25, vantagem_principal: 'Redução LDL-c 40–55%. Melhor custo-benefício. NNT 19–25 em prevenção secundária (HPS)', desvantagem_principal: 'Mialgia 5–10%. Diabetes novo-início (alta dose)', contraindicacoes_principais: ['Hepatopatia ativa', 'Gravidez', 'Rabdomiólise prévia'], quando_preferir: 'Prevenção CV secundária, pós-SCA, muito alto risco CV (meta LDL < 55)' },
      { linha: '1a_linha', molecula: 'Rosuvastatina 20–40mg', classe: 'Estatina alta intensidade', indicacao: 'Alto risco CV, LDL muito elevado, prevenção primária com PCR-us alta (JUPITER)', nnt: 25, forca_evidencia: 'A', diretriz: 'ESC/EAS 2019', disponivel_sus: false, custo_estimado_mes: 60, vantagem_principal: 'Maior potência por mg (7× sinvastatina). Menor metabolismo CYP3A4 (menos interações)', desvantagem_principal: 'Menor genérico acessível que atorvastatina', contraindicacoes_principais: ['Hepatopatia ativa', 'Miopatia', 'Gravidez'], quando_preferir: 'Prevenção primária + PCR-us ≥ 2 mg/L, LDL > 190 mg/dL, hipersensibilidade a atorvastatina' },
      { linha: '2a_linha', molecula: 'Ezetimiba 10mg', classe: 'Inibidor absorção de colesterol', indicacao: 'LDL-c não controlado em estatina máxima tolerada', nnt: 50, forca_evidencia: 'A', diretriz: 'ESC/EAS 2019 / SBC 2023', disponivel_sus: false, custo_estimado_mes: 80, vantagem_principal: 'Redução adicional LDL-c 15–20%. Excelente tolerabilidade (IMPROVE-IT — 18.144 pacientes)', desvantagem_principal: 'Benefício absoluto modesto isolado. Custo moderado.', contraindicacoes_principais: ['Gravidez', 'Hepatopatia ativa'], quando_preferir: 'Adição à estatina quando LDL-c > meta. Intolerância à estatina de alta dose' },
      { linha: '3a_linha', molecula: 'Evolocumabe 140mg sc / Alirocumabe 75–150mg sc', classe: 'Inibidor PCSK9', indicacao: 'LDL-c > 70 mg/dL apesar de estatina + ezetimiba. Hipercolesterolemia familiar.', nnt: 67, forca_evidencia: 'A', diretriz: 'ESC/EAS 2019', disponivel_sus: false, custo_estimado_mes: 3800, vantagem_principal: 'Redução LDL-c 50–60% adicional. FOURIER/ODYSSEY: 15% RRR MACE. Seguro e bem tolerado', desvantagem_principal: 'Custo muito elevado (~R$3.500–5.000/mês). Injetável a cada 2–4 semanas', contraindicacoes_principais: ['Hipersensibilidade ao anticorpo', 'Gravidez (segurança não estabelecida)'], quando_preferir: 'Hipercolesterolemia familiar, muito alto risco + LDL > 70 apesar de estatina + ezetimiba' },
    ],
  },
  I48: {
    condicao: 'Fibrilação Atrial Não Valvular',
    nota: 'Anticoagulação indicada em CHA₂DS₂-VASc ≥ 2 (homens) / ≥ 3 (mulheres). NOACs preferíveis à varfarina em FA não-valvular. Avaliar risco de sangramento (HAS-BLED). Controle de FC alvo < 110 bpm (leniente) ou < 80 bpm (estrito, se sintomático).',
    alternativas: [
      { linha: '1a_linha', molecula: 'Apixabana 5mg 2×/dia', classe: 'NOAC — inibidor Xa', indicacao: 'FA não-valvular + CHA₂DS₂-VASc ≥ 2', nnt: 171, forca_evidencia: 'A', diretriz: 'ESC 2020 / SBC 2024', disponivel_sus: false, custo_estimado_mes: 200, vantagem_principal: 'Único NOAC com redução simultânea de AVC + sangramento maior + mortalidade (ARISTOTLE). Menor hemorragia intracraniana', desvantagem_principal: '2 comprimidos 2×/dia. Custo. Contraindicado TFG < 15', contraindicacoes_principais: ['Valvopatia mitral moderada-grave', 'Prótese mecânica', 'TFG < 15'], quando_preferir: 'FA não-valvular + CHA₂DS₂-VASc ≥ 2. Idosos. DRC moderada.' },
      { linha: '1a_linha', molecula: 'Rivaroxabana 20mg/dia', classe: 'NOAC — inibidor Xa', indicacao: 'FA não-valvular + CHA₂DS₂-VASc ≥ 2', forca_evidencia: 'A', diretriz: 'ESC 2020', disponivel_sus: false, custo_estimado_mes: 180, vantagem_principal: 'Dose única diária — melhor aderência. Estudado em pacientes de muito alto risco (ROCKET AF CHADS₂ médio 3,5)', desvantagem_principal: 'Sangramento GI aumentado vs. varfarina. Tomado com jantar (absorção)', contraindicacoes_principais: ['Valvopatia mitral', 'Prótese mecânica', 'TFG < 15'], quando_preferir: 'Paciente com risco de baixa aderência a 2×/dia. Alto risco trombótico.' },
      { linha: '1a_linha', molecula: 'Dabigatrana 150mg 2×/dia', classe: 'NOAC — inibidor trombina', indicacao: 'FA não-valvular, especialmente < 75 anos', nnt: 156, forca_evidencia: 'A', diretriz: 'ESC 2020', disponivel_sus: false, custo_estimado_mes: 185, vantagem_principal: 'Maior redução de AVC isquêmico que varfarina (150 mg). Antídoto disponível (idarucizumabe)', desvantagem_principal: 'Dispepsia 11%. Contraindicado TFG < 30. Sangramento GI aumentado', contraindicacoes_principais: ['TFG < 30', 'Prótese mecânica', 'Valvopatia mitral'], quando_preferir: 'FA + CHA₂DS₂-VASc alto + < 75 anos sem DRC moderada-grave' },
      { linha: '2a_linha', molecula: 'Varfarina 2,5–10mg (INR 2,0–3,0)', classe: 'Antagonista vitamina K', indicacao: 'FA valvular (estenose mitral) ou prótese mecânica — NOACs contraindicados', forca_evidencia: 'A', diretriz: 'ESC 2020', disponivel_sus: true, custo_estimado_mes: 15, vantagem_principal: 'Única opção em valvopatia mitral + prótese mecânica. Baixo custo', desvantagem_principal: 'Monitoramento INR obrigatório. Múltiplas interações. Janela terapêutica estreita.', contraindicacoes_principais: ['Gravidez (1º e 3º trimestres)', 'HAS não controlada', 'Sangramento ativo'], quando_preferir: 'Valvopatia mitral, prótese mecânica, TFG < 15 (onde NOACs contraindicados)' },
      { linha: '2a_linha', molecula: 'Metoprolol succinato 25–200mg / Carvedilol 12,5–50mg', classe: 'Betabloqueador — controle de FC', indicacao: 'FA + controle de FC. Especialmente FA + IC-FEr', forca_evidencia: 'A', diretriz: 'ESC 2020', disponivel_sus: true, custo_estimado_mes: 25, vantagem_principal: 'Controle FC eficaz + benefício em IC-FEr. Custo mínimo', desvantagem_principal: 'Bradicardia, BAV, broncoespasmo', contraindicacoes_principais: ['Asma grave', 'BAV 2–3º', 'Bradicardia sintomática'], quando_preferir: 'FA + IC-FEr (carvedilol). FA + HAS + sem IC (metoprolol). Alvo FC < 110 bpm' },
    ],
  },
  I21: {
    condicao: 'Síndrome Coronariana Aguda / IAM',
    nota: 'Iniciar DAPT imediatamente (AAS 300 mg + P2Y12i). Ticagrelor 90 mg 2×/dia preferível ao clopidogrel em IAM-CSST e IAMSSST de alto risco. Estatina de alta intensidade nas primeiras 24–96h. IECA + betabloqueador se FEVE ≤ 40%. Manter DAPT por 12 meses.',
    alternativas: [
      { linha: '1a_linha', molecula: 'Ticagrelor 90mg 2×/dia + AAS 100mg', classe: 'DAPT — P2Y12i de ação direta', indicacao: 'SCA (IAM-CSST, IAMSSST de alto risco)', nnt: 52, forca_evidencia: 'A', diretriz: 'ESC 2023 / SBC 2024', disponivel_sus: false, custo_estimado_mes: 180, vantagem_principal: 'Superior ao clopidogrel: RRR 16% MACE + redução mortalidade 22% (PLATO). Não pró-droga (inibição mais rápida)', desvantagem_principal: 'Dispneia transitória 13%. Custo. 2×/dia', contraindicacoes_principais: ['AVC hemorrágico prévio', 'Sangramento ativo', 'Usar AAS ≤ 100 mg (AAS alta dose reduz benefício do ticagrelor)'], quando_preferir: 'IAM-CSST / IAMSSST de alto risco. Padrão recomendado pelas diretrizes.' },
      { linha: '1a_linha', molecula: 'Clopidogrel 75mg + AAS 100mg', classe: 'DAPT — P2Y12i pró-droga', indicacao: 'SCA quando ticagrelor/prasugrel contraindicados ou indisponíveis', nnt: 48, forca_evidencia: 'A', diretriz: 'ESC 2023', disponivel_sus: true, custo_estimado_mes: 20, vantagem_principal: 'Custo mínimo. SUS. Ampla experiência clínica. Estabeleceu DAPT (CURE)', desvantagem_principal: 'Inferior a ticagrelor/prasugrel. Variabilidade genética (CYP2C19). Resposta em 6–8h', contraindicacoes_principais: ['Sangramento ativo'], quando_preferir: 'Quando ticagrelor contraindicado ou indisponível. Alto risco de sangramento. Pós-AVC recente.' },
      { linha: '1a_linha', molecula: 'Atorvastatina 80mg', classe: 'Estatina alta intensidade', indicacao: 'Todo paciente com SCA — iniciar nas primeiras 24–96h', nnt: 26, forca_evidencia: 'A', diretriz: 'ESC 2023 / SBC 2024', disponivel_sus: true, custo_estimado_mes: 25, vantagem_principal: 'PROVE IT: RRR 16% vs. pravastatina 40 mg. Efeitos pleiotrópicos (estabilização de placa). Meta LDL < 55 mg/dL', desvantagem_principal: 'Mialgia (raro em doses altas). Elevação TGO/TGP em 3% em doses altas', contraindicacoes_principais: ['Hepatopatia ativa', 'Gravidez'], quando_preferir: 'Todo paciente com SCA — iniciar imediatamente, independente do LDL basal' },
      { linha: '2a_linha', molecula: 'Bisoprolol 2,5–10mg / Carvedilol 12,5–50mg', classe: 'Betabloqueador', indicacao: 'Pós-IAM com FEVE ≤ 40% ou IC', nnt: 18, forca_evidencia: 'A', diretriz: 'ESC 2023', disponivel_sus: true, custo_estimado_mes: 25, vantagem_principal: 'Redução mortalidade 34% em IC-FEr pós-IAM (COPERNICUS). Antiisquêmico, antiarrítmico', desvantagem_principal: 'Bradicardia, BAV, broncoespasmo. Iniciar com dose mínima.', contraindicacoes_principais: ['Asma grave', 'BAV 2–3º', 'Choque cardiogênico ativo'], quando_preferir: 'FEVE ≤ 40% pós-IAM, IC, FA associada, hipertensão' },
    ],
  },
  J44: {
    condicao: 'Doença Pulmonar Obstrutiva Crônica (DPOC)',
    nota: 'LAMA é 1ª linha para maioria dos pacientes (GOLD B/E). ICS reservado para eosinófilos ≥ 300 cél/µL ou ≥ 2 exacerbações/ano. Terapia tripla (LAMA+LABA+ICS) em não controlados. Cessar tabagismo: maior impacto modificável na progressão.',
    alternativas: [
      { linha: '1a_linha', molecula: 'Tiotrópio 18mcg/dia (Spiriva)', classe: 'LAMA', indicacao: 'DPOC sintomático (mMRC ≥ 1) — GOLD B e E', nnt: 8, forca_evidencia: 'A', diretriz: 'GOLD 2024', disponivel_sus: false, custo_estimado_mes: 180, vantagem_principal: 'Redução exacerbações 14%, hospitalização 14% (UPLIFT). Dose única diária. Referência na classe', desvantagem_principal: 'Boca seca 14%. Custo (sem genérico acessível no Brasil)', contraindicacoes_principais: ['Glaucoma ângulo fechado', 'HPB com retenção urinária', 'Hipersensibilidade a atropina'], quando_preferir: 'DPOC Gold B/E sintomático como monoterapia ou base de terapia combinada' },
      { linha: '1a_linha', molecula: 'Formoterol/Budesonida 6/200mcg 2×/dia', classe: 'LABA+ICS', indicacao: 'DPOC com eosinófilos ≥ 300 cél/µL ou ≥ 2 exacerbações/ano', forca_evidencia: 'A', diretriz: 'GOLD 2024', disponivel_sus: true, custo_estimado_mes: 90, vantagem_principal: 'Redução exacerbações 25–30% vs. LABA isolado em eosinofílicos. Budesonida: menor risco de pneumonia que fluticasona', desvantagem_principal: 'Pneumonia aumentada vs. LAMA+LABA. Não indicado sem eosinofilia', contraindicacoes_principais: ['DPOC sem eosinofilia < 100/µL', 'TBC ativa'], quando_preferir: 'DPOC + eosinofilia ≥ 300 cél/µL e/ou exacerbações frequentes' },
      { linha: '2a_linha', molecula: 'Fluticasona/Umeclidínio/Vilanterol 92/55/22mcg', classe: 'LAMA+LABA+ICS (tripla fixo)', indicacao: 'DPOC não controlado em dupla terapia', nnt: 7, forca_evidencia: 'A', diretriz: 'GOLD 2024', disponivel_sus: false, custo_estimado_mes: 350, vantagem_principal: 'IMPACT: maior redução de exacerbações entre todas as estratégias. Redução mortalidade 28% vs. LABA+ICS (exploratório)', desvantagem_principal: 'Custo elevado. Pneumonia 8% vs. 5% (sem ICS)', contraindicacoes_principais: ['DPOC sem eosinofilia (risco de pneumonia sem benefício)', 'TBC ativa'], quando_preferir: 'DPOC Gold E em dupla terapia + eosinófilos ≥ 300 ou ≥ 2 exacerbações moderadas/graves' },
      { linha: '3a_linha', molecula: 'Roflumilaste 500mcg/dia', classe: 'Inibidor PDE4', indicacao: 'DPOC grave (VEF1 < 50%) + bronquite crônica + ≥ 2 exacerbações/ano', forca_evidencia: 'B', diretriz: 'GOLD 2024', disponivel_sus: false, custo_estimado_mes: 220, vantagem_principal: 'Redução adicional de exacerbações 17% em fenótipo bronquite crônica grave (REACT)', desvantagem_principal: 'Náusea, diarreia, perda de peso, insônia. Oral (não inalado). Sem efeito broncodilatador', contraindicacoes_principais: ['Depressão grave', 'Hepatopatia moderada-grave', 'Peso < 60 kg (risco de perda ponderal)'], quando_preferir: 'DPOC grave + bronquite crônica + exacerbações frequentes apesar de tripla inalatória' },
    ],
  },
  N18: {
    condicao: 'Doença Renal Crônica',
    nota: 'Controle de PA < 130/80 mmHg. IECA/BRA em proteinúria (suspender se K+ > 5,5 ou creatinina ↑ > 30%). iSGLT2 indicado em DM2+DRC TFG ≥ 20 (DAPA-CKD/CREDENCE). Finerenona em DM2+DRC com K+ < 5,0. Evitar AINEs, contraste iodado sem hidratação.',
    alternativas: [
      { linha: '1a_linha', molecula: 'Enalapril 10–20mg / Losartana 50–100mg', classe: 'IECA / BRA', indicacao: 'DRC + proteinúria (UACR > 30 mg/g). Redução da progressão renal', nnt: 24, forca_evidencia: 'A', diretriz: 'KDIGO 2024', disponivel_sus: true, custo_estimado_mes: 15, vantagem_principal: 'Nefroproteção (RENAAL: redução 16% desfecho renal). Controle de PA + redução de proteinúria. Custo mínimo', desvantagem_principal: 'Monitorar K+ + creatinina. Tosse (IECA). Contraindicado gestação', contraindicacoes_principais: ['K+ > 5,5', 'TFG < 15 (relativa)', 'Gravidez', 'Estenose bilateral AR'], quando_preferir: 'Toda DRC com proteinúria ≥ 300 mg/g. DM2 + nefropatia. DRC + HAS.' },
      { linha: '1a_linha', molecula: 'Dapagliflozina 10mg', classe: 'iSGLT2', indicacao: 'DRC G2–G4 (TFG 20–75) + proteinúria (DAPA-CKD — independente de DM2)', nnt: 19, forca_evidencia: 'A', diretriz: 'KDIGO 2024', disponivel_sus: false, custo_estimado_mes: 180, vantagem_principal: 'DAPA-CKD: 39% redução desfecho renal composto. Benefício independente de DM2. Redução CV e mortalidade', desvantagem_principal: 'Custo. Queda transitória de TFGe (esperada). ITU genital', contraindicacoes_principais: ['TFG < 20', 'Diálise', 'DM1', 'Cetoacidose'], quando_preferir: 'DRC TFG 20–75 + proteinúria, com ou sem DM2 (KDIGO 1ª linha com iSGLT2)' },
      { linha: '2a_linha', molecula: 'Finerenona 10–20mg', classe: 'Antagonista mineralocorticoide não esteroidal', indicacao: 'DM2 + DRC + K+ < 5,0 + em IECA/BRA ± iSGLT2', nnt: 29, forca_evidencia: 'A', diretriz: 'KDIGO 2024', disponivel_sus: false, custo_estimado_mes: 320, vantagem_principal: 'FIDELIO/FIGARO: 18–23% redução renal + 13–14% CV. Sem ginecomastia. Benefício em DRC 1–4', desvantagem_principal: 'Hiperpotassemia (9,5%). Custo elevado. Monitorar K+ em 4 semanas', contraindicacoes_principais: ['K+ > 5,0 no início', 'TFG < 25', 'Insuficiência adrenal', 'IECA+BRA concomitante'], quando_preferir: 'DM2 + DRC + K+ < 5,0 + em IECA/BRA + iSGLT2 ou quando iSGLT2 contraindicado' },
      { linha: '3a_linha', molecula: 'Bicarbonato de sódio 650–1950mg/dia', classe: 'Alcalinizante', indicacao: 'DRC G3–G5 + acidose metabólica (bicarbonato < 22 mEq/L)', forca_evidencia: 'B', diretriz: 'KDIGO 2024', disponivel_sus: true, custo_estimado_mes: 20, vantagem_principal: 'Retarda progressão renal e reduz hipercatabolismo muscular em DRC com acidose', desvantagem_principal: 'Expansão de volume (HAS). Edema. Retenção de Na+.', contraindicacoes_principais: ['Edema / congestão', 'Hipernatremia'], quando_preferir: 'DRC G3b–G5 + HCO3- < 22 mEq/L persistente, sem sobrecarga volêmica' },
    ],
  },
  F32: {
    condicao: 'Transtorno Depressivo Maior',
    nota: 'ISRS são 1ª linha universais (menor efeitos adversos). Escitalopram e sertralina: melhor perfil eficácia-tolerabilidade (Cipriani 2018). Resposta esperada em 4–6 semanas; remissão em 8–12 semanas. Manutenção por ≥ 6–12 meses após remissão (> 1 episódio: 2+ anos). Associar TCC quando disponível (superior à monoterapia em moderada-grave).',
    alternativas: [
      { linha: '1a_linha', molecula: 'Sertralina 50–200mg', classe: 'ISRS', indicacao: 'Depressão moderada-grave, TAG, pânico, TOC, TEPT', nnt: 6, forca_evidencia: 'A', diretriz: 'APA 2024 / CFM 2023', disponivel_sus: true, custo_estimado_mes: 20, vantagem_principal: 'Melhor balanço eficácia-tolerabilidade (Cipriani 2018). Custo mínimo. SUS. Segura em gestação (menor risco fetal)', desvantagem_principal: 'Disfunção sexual 20–30%. Insônia, náusea inicial. Síndrome de descontinuação moderada', contraindicacoes_principais: ['IMAOs (14 dias)', 'Tramadol (síndrome serotoninérgica)', 'Pimozida'], quando_preferir: 'Depressão maior — 1ª escolha universal. Gestante. Idosos. Depressão + ansiedade.' },
      { linha: '1a_linha', molecula: 'Escitalopram 10–20mg', classe: 'ISRS', indicacao: 'Depressão, TAG', nnt: 7, forca_evidencia: 'A', diretriz: 'APA 2024', disponivel_sus: false, custo_estimado_mes: 50, vantagem_principal: 'Melhor eficácia absoluta (Cipriani 2018, OR 1,97). Interações mínimas (menos inibição CYP)', desvantagem_principal: 'Custo moderado. Prolongamento QT em dose máxima + idoso', contraindicacoes_principais: ['IMAOs', 'Citalopram concomitante', 'Prolongamento QT + dose máxima (> 60 anos: máx 10 mg)'], quando_preferir: 'Depressão maior — quando mínimas interações são prioritárias. Comorbidade GAD.' },
      { linha: '2a_linha', molecula: 'Venlafaxina XR 75–225mg', classe: 'SNRI', indicacao: 'Falha a ISRS, depressão + dor crônica, depressão com sintomas ansiosos graves, TAG', forca_evidencia: 'A', diretriz: 'APA 2024', disponivel_sus: false, custo_estimado_mes: 90, vantagem_principal: 'Maior eficácia absoluta que a maioria dos ISRS (Cipriani: OR 2,03). Efeito dual (5-HT + NA). Útil em dor crônica', desvantagem_principal: 'Síndrome de descontinuação grave. Elevação de PA em alta dose. Náusea inicial', contraindicacoes_principais: ['IMAOs', 'HAS não controlada', 'Glaucoma ângulo fechado'], quando_preferir: 'Falha a 1 ou 2 ISRS. Depressão + dor neuropática. TAG moderado-grave com componente somático.' },
      { linha: '2a_linha', molecula: 'Bupropiona XL 150–300mg', classe: 'NDRI (inibidor NA+DA)', indicacao: 'Depressão + cessação tabágica, depressão com hipersônia/fadiga, depressão sem disfunção sexual', forca_evidencia: 'A', diretriz: 'APA 2024', disponivel_sus: false, custo_estimado_mes: 120, vantagem_principal: 'Única opção sem disfunção sexual (até melhora). Perda de peso leve. Eficaz na cessação tabágica', desvantagem_principal: 'Aumenta limiar convulsivo (risco de crise). Insônia. Não usar em bulimia', contraindicacoes_principais: ['Convulsão / epilepsia', 'Bulimia/anorexia', 'Retirada abrupta de BZD ou álcool', 'IMAOs'], quando_preferir: 'Disfunção sexual por ISRS. Depressão + sonolência/fadiga. Paciente fumante.' },
      { linha: '3a_linha', molecula: 'Amitriptilina 25–150mg', classe: 'Antidepressivo tricíclico', indicacao: 'Depressão refratária, dor neuropática, enxaqueca profilaxia, fibromialgia', forca_evidencia: 'A', diretriz: 'APA 2024', disponivel_sus: true, custo_estimado_mes: 10, vantagem_principal: 'Alta eficácia (Cipriani: 2ª maior OR após amitriptilina). Custo mínimo. Dupla indicação (dor + depressão)', desvantagem_principal: 'Anticolinérgico (boca seca, obstipação, retenção urinária). Sedação intensa. Cardiotóxico em superdosagem', contraindicacoes_principais: ['Idosos (Beers 2023 — evitar)', 'IAM recente', 'BAV', 'Glaucoma ângulo fechado', 'Risco suicídio (cardiotóxico)'], quando_preferir: 'Depressão + dor neuropática/fibromialgia. Depressão refratária. Adultos jovens sem comorbidades cardíacas.' },
    ],
  },
  M81: {
    condicao: 'Osteoporose',
    nota: 'Indicar tratamento farmacológico: T-score ≤ -2,5 (coluna/fêmur) OU fratura por fragilidade prévia OU alto risco FRAX (≥ 20% risco de fratura maior em 10 anos). Calcio (1000–1200 mg/dia) + vitamina D3 (800–2000 UI/dia) como adjuvante universal. Alendronato: 1ª linha oral (SUS).',
    alternativas: [
      { linha: '1a_linha', molecula: 'Alendronato 70mg/semana', classe: 'Bisfosfonato oral', indicacao: 'Osteoporose confirmada (T-score ≤ -2,5) ou fratura por fragilidade', nnt: 20, forca_evidencia: 'A', diretriz: 'SBEM 2022 / NOF 2023', disponivel_sus: true, custo_estimado_mes: 15, vantagem_principal: 'FIT: redução 51% fraturas de quadril e 55% vertebrais. Custo mínimo. SUS. 40+ anos de evidência', desvantagem_principal: 'Esofagite (tomar em jejum + 30 min em pé). Contraindicado TFG < 35', contraindicacoes_principais: ['TFG < 35 mL/min', 'Acalasia/esofagite ativa', 'Hipocalcemia', 'Gravidez'], quando_preferir: 'Osteoporose pós-menopáusica, homens > 50 anos com osteoporose, 1ª linha universal' },
      { linha: '1a_linha', molecula: 'Ácido zoledrônico 5mg IV/ano', classe: 'Bisfosfonato IV', indicacao: 'Osteoporose com intolerância oral, pós-fratura quadril, má-aderência oral', nnt: 14, forca_evidencia: 'A', diretriz: 'SBEM 2022', disponivel_sus: false, custo_estimado_mes: 180, vantagem_principal: 'HORIZON-PFT: redução 70% fraturas vertebrais, 41% quadril. Único a reduzir mortalidade geral (28%). Dose anual IV — máxima aderência', desvantagem_principal: 'Reação de fase aguda 32% (febre, mialgia 24–72h após 1ª infusão). Infraestrutura IV necessária', contraindicacoes_principais: ['TFG < 30 mL/min', 'Hipocalcemia', 'Gravidez'], quando_preferir: 'Intolerância a oral, má aderência, pós-fratura de quadril, TFG 30–60' },
      { linha: '2a_linha', molecula: 'Denosumabe 60mg SC a cada 6 meses', classe: 'Anticorpo anti-RANKL', indicacao: 'TFG < 35 (bisfosfonatos contraindicados), intolerância ou falha a bisfosfonatos', nnt: 16, forca_evidencia: 'A', diretriz: 'SBEM 2022', disponivel_sus: false, custo_estimado_mes: 600, vantagem_principal: 'FREEDOM: redução 68% vertebral, 40% quadril. Eficaz em TFG < 35. SC 2×/ano', desvantagem_principal: 'Rebote de fratura vertebral múltipla ao descontinuar sem transição (RISCO GRAVE). Hipocalcemia em DRC.', contraindicacoes_principais: ['Hipocalcemia não corrigida', 'Infecção ativa grave', 'Gravidez'], quando_preferir: 'DRC TFG < 35 (bisfosfonatos contraindicados). Intolerância a bisfosfonatos. Alto risco FRAX + aderência difícil' },
    ],
  },
  I63: {
    condicao: 'AVC Isquêmico — Prevenção Secundária',
    nota: 'AIT/AVC pequeno vaso: DAPT (AAS+clopidogrel) por 21–90 dias, depois antiplaquetário único. AVC cardioembólico (FA): NOAC (iniciar após 2–14 dias). AVC aterosclerótico: estatina alta intensidade + antiplaquetário. Controle intensivo da PA (< 130/80 mmHg) para prevenção secundária.',
    alternativas: [
      { linha: '1a_linha', molecula: 'Clopidogrel 75mg + AAS 100mg (21 dias)', classe: 'DAPT', indicacao: 'AIT alto risco (ABCD2 ≥ 4) ou AVC leve (NIHSS ≤ 3) — iniciar ≤ 24h', nnt: 29, forca_evidencia: 'A', diretriz: 'SBC/ABN 2023 / AHA/ASA 2021', disponivel_sus: true, custo_estimado_mes: 20, vantagem_principal: 'CHANCE + POINT: RRR 25–32% AVC recorrente. Benefício nas primeiras 2 semanas. Custo mínimo', desvantagem_principal: 'Apenas 21 dias (risco sangramento após). Não usar em AVC grave ou cardioembólico', contraindicacoes_principais: ['AVC hemorrágico', 'NIHSS > 5 (AVC grave)', 'Anticoagulação indicada (FA)'], quando_preferir: 'AIT de alto risco ou AVC isquêmico leve — iniciar em ≤ 24h. Parar DAPT após 21 dias' },
      { linha: '1a_linha', molecula: 'Apixabana 5mg 2×/dia (se FA)', classe: 'NOAC', indicacao: 'AVC/AIT isquêmico + FA não-valvular confirmada', nnt: 171, forca_evidencia: 'A', diretriz: 'ESC 2020 / SBC/ABN 2023', disponivel_sus: false, custo_estimado_mes: 200, vantagem_principal: 'ARISTOTLE: menor HIC + menor AVC + menor mortalidade vs. varfarina. Iniciar 7–14 dias pós-AVC moderado', desvantagem_principal: 'Custo. Contraindica bisfosfonatos/AAS concomitante em alto risco hemorrágico', contraindicacoes_principais: ['Valvopatia mitral', 'Prótese mecânica', 'TFG < 15'], quando_preferir: 'AVC isquêmico/AIT + FA não-valvular. Iniciar após 2–14 dias conforme tamanho do infarto' },
      { linha: '1a_linha', molecula: 'Atorvastatina 40–80mg', classe: 'Estatina alta intensidade', indicacao: 'Todo AVC isquêmico aterosclerótico — independente do LDL', forca_evidencia: 'A', diretriz: 'SBC/ABN 2023', disponivel_sus: true, custo_estimado_mes: 25, vantagem_principal: 'SPARCL (2006): atorvastatina 80 mg reduziu AVC recorrente em 16%. Meta LDL < 70 mg/dL em AVC aterosclerótico', desvantagem_principal: 'Mialgia. Ligeiro aumento de AVC hemorrágico (SPARCL — contexto específico)', contraindicacoes_principais: ['Hepatopatia ativa', 'Gravidez'], quando_preferir: 'AVC isquêmico aterosclerótico (LAA, lacunar) — estatina de alta intensidade imediatamente' },
      { linha: '2a_linha', molecula: 'Clopidogrel 75mg (monoterapia longa)', classe: 'Antiplaquetário P2Y12', indicacao: 'AVC isquêmico não cardioembólico — manutenção após DAPT curta (21 dias)', forca_evidencia: 'A', diretriz: 'SBC/ABN 2023', disponivel_sus: true, custo_estimado_mes: 20, vantagem_principal: 'CAPRIE: levemente superior ao AAS em prevenção AVC/IAM/morte vascular. Custo baixo. Sem ulcero-gastrolesão', desvantagem_principal: 'Variabilidade genética CYP2C19 (metabolizadores lentos — menor efeito)', contraindicacoes_principais: ['Sangramento ativo'], quando_preferir: 'Manutenção após DAPT curta. AVC + alto risco GI (substituto ao AAS). Preferível se tiver intolerância ao AAS' },
    ],
  },
  J18: {
    condicao: 'Pneumonia Adquirida na Comunidade (PAC)',
    nota: 'Estratificação por gravidade (PSI/PORT ou CURB-65) define local de tratamento (ambulatorial / internação / UTI). Ambulatorial: amoxicilina ± macrolídeo. Internação: beta-lactâmico + macrolídeo ou monoterapia com fluoroquinolona respiratória. UTI: beta-lactâmico IV + azitromicina IV (ou fluoroquinolona IV).',
    alternativas: [
      { linha: '1a_linha', molecula: 'Amoxicilina 500–1000mg 3×/dia (5–7 dias)', classe: 'Betalactâmico — Aminopenicilina', indicacao: 'PAC leve ambulatorial sem comorbidades', forca_evidencia: 'A', diretriz: 'SBPT/ATS/IDSA 2019', disponivel_sus: true, custo_estimado_mes: 20, vantagem_principal: 'Cobertura S. pneumoniae (principal patógeno). Custo mínimo. SUS. BEA seguro em gestação', desvantagem_principal: 'Não cobre atípicos (Mycoplasma, Chlamydophila). Resistência crescente', contraindicacoes_principais: ['Alergia a penicilinas', 'Suspeita de Legionella', 'Falha prévia'], quando_preferir: 'PAC leve, sem fatores de risco para resistência, ambulatorial CURB-65 = 0–1' },
      { linha: '1a_linha', molecula: 'Amoxicilina-Clavulanato 875/125mg 2×/dia + Azitromicina 500mg/dia (5 dias)', classe: 'Betalactâmico + Macrolídeo', indicacao: 'PAC ambulatorial com comorbidades (DPOC, DM, IC, DRC, imunossupressão)', forca_evidencia: 'A', diretriz: 'ATS/IDSA 2019', disponivel_sus: true, custo_estimado_mes: 50, vantagem_principal: 'Cobertura ampla: S. pneumoniae + H. influenzae + atípicos. Padrão para PAC com comorbidades', desvantagem_principal: 'Diarreia 10–15%. Náusea. Custo moderado', contraindicacoes_principais: ['Hepatotoxicidade por amoxicilina-clavulanato prévia', 'Prolongamento QT (azitromicina)'], quando_preferir: 'PAC ambulatorial com comorbidades (DPOC, DM, IC) ou suspeita de atípicos' },
      { linha: '2a_linha', molecula: 'Levofloxacino 500mg/dia ou Moxifloxacino 400mg/dia (5 dias)', classe: 'Fluoroquinolona respiratória', indicacao: 'PAC ambulatorial com falha a betalactâmico ou alergia, internação moderada', forca_evidencia: 'A', diretriz: 'ATS/IDSA 2019', disponivel_sus: false, custo_estimado_mes: 80, vantagem_principal: 'Monoterapia que cobre S. pneumoniae + atípicos + anaeróbios. Dose única diária', desvantagem_principal: 'Risco de tendinite/ruptura tendão. Prolongamento QT. Seleciona resistência. Custo', contraindicacoes_principais: ['< 18 anos', 'Gravidez', 'Prolongamento QT congênito', 'Uso prévio de quinolona nos últimos 3 meses (resistência)'], quando_preferir: 'Falha a betalactâmico, alergia a penicilina grave, PAC com comorbidades sem acesso a combinação' },
    ],
  },
};
